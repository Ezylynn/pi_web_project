...
class Student{
    ...
    static async fetchEssentials(){
        const client = await pool.connect();
        try{
            const userInfo = await client.query("SELECT students_info.student_id, users.user_id, users.role, users.username, students_info.full_name, students_info.grade, students_info.email, test_results.answer, test_results.status, users.created_at, test_results.attempted_at FROM students_info FULL JOIN users ON users.user_id = students_info.student_id FULL JOIN test_results ON students_info.student_id = test_results.student_id;");
            if (userInfo.rows.length > 0){
